...
public class MainController {
	 ...
	 */
	  ...
	  mailSender.mailSendWithUserKey(MailVo.getFriId(), request);
	  System.out.println("fridid:"+MailVo.getFriId());
	  ...
	  public String key_alterConfirm(@RequestParam("FriId") String
	   FriId, @RequestParam("status_yn") String status_yn) {
	  
	  mailSender.alter_userKey_service(FriId, status_yn); // mailsender의 경우 @Autowired
	...
	/*
	 * @RequestMapping(value = "mailSending.mb", method = RequestMethod.GET) //
	 * 회원가입창 보기 public String registerGet() throws Exception {
	 * logger.info("register user get...."); return "/friends/addfriends"; }
	 * 
	 * @RequestMapping(value = "mailSending.mb", method = RequestMethod.POST) public
	 * String registerPost(MailVo vo, RedirectAttributes rttr) throws Exception { //
	 * 회원가입하기 logger.info("register user post...."); MailSendService.create(vo);
	 * rttr.addFlashAttribute("msg", "regSuccess"); return "redirect:/"; }
	 * 
	 * @RequestMapping(value = "/emailConfirm", method = RequestMethod.GET) public
	 * String emailConfirm(String FRI_ID, Model model) throws Exception {
	 * service.userAuth(FRI_ID); model.addAttribute("userEmail", FRI_ID);
	 * 
	 * return "/friends/emailConfirm"; }
	 * 
	 */
